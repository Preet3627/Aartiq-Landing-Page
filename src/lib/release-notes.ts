...
export const releases: ReleaseEntry[] = [
  {
    ...
    changes: {
      new: [
        'Claude Desktop MCP server rewritten — 64 tools across 11 categories (AI chat, tabs, bookmarks, history, settings, scheduling, permissions, security, panels, app knowledge)',
        'MCP HTTP bridge architecture — stdio MCP server communicates with running Aartiq browser over HTTP (port 46203), no standalone fallback',
        'MCP AI sidebar integration — send_ai_prompt goes through the actual sidebar for full capabilities: RAG, web search, PDF generation, navigation, structured output',
      ...
      fix: [
        'Fixed prompt handler to wait 5s for renderer sidebar pickup before falling back to dumb AiGateway path',
      ...
      change: [
        ...
        'Removed duplicate inline HTML template in mcp-browser-server.js',
      ],
      docs: [
        'Rewrote README with 64-tool MCP table, updated architecture diagram, directory layout',
